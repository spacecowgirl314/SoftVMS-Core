...
BIG unsigned char flashmem[0x20000];
...
{
    int r=0, t=0;
    memset(flashmem, 0, sizeof(flashmem));
    while(t<sizeof(flashmem) && (r=read(fd, flashmem+t, sizeof(flashmem)-t))>0)
        t += r;
    if(r<0 || t<0x480)
        return 0;
    return t;
...
void fakeflash(char *filename, int sz)
{
    ...
    if((fn2 = strrchr(filename, '/'))!=NULL)
        ...
        filename = fn2+1;
    memset(flashmem+241*512, 0, sizeof(flashmem)-241*512);
    root = flashmem+255*512;
    fat = flashmem+254*512;
    dir = flashmem+253*512;
    sz = ((sz+511)>>9);
    for(i=0; i<256*2; i+=2) {
        fat[i] = 0xfc;
        fat[i+1] = 0xff;
...
void check_gamesize()
{
    ...
    if(fatblk>=256 || dircnt>=256 || !dircnt)
        return;
    fat = flashmem+fatblk*512;
    while(dircnt-- && dirblk<256) {
        dir = flashmem+dirblk*512;
...
int loadflash(char *filename)
{
    ...
    if((fd = open(filename, O_RDONLY|O_BINARY))>=0) {
        if(!(r=initflash(fd))) {
            close(fd);
            error_msg("%s: can't load flashmem image", filename);
            return 0;
        }
        close(fd);
        gamesize = r;
    ...
    switch(pc) {
        case 0x100:
        {
            ...
            if(a>=gamesize)
                ...
                writemem(0x100, 0x00);
                for(i=0; i<0x80; i++)
                    flashmem[(a&~0xff)|((a+i)&0xff)] = ram[1][i+0x80];
#ifdef __DC__
                if(!flashmem_written(a))
                    writemem(0x100, 0xff);
#endif
            }
            /*
        ...
        case 0x110:
        {
            ...
            int r = 0;
            for(i=0; i<0x80; i++)
                if((r = (flashmem[(a&~0xff)|((a+i)&0xff)] ^ ram[1][i+0x80])) != 0)
                    break;
            writemem(0x100, r);
        }
            return 0x115;
        case 0x120:
        {
            int i, a = ((ram[1][0x7d]<<16)|(ram[1][0x7e]<<8)|ram[1][0x7f])&0x1ffff;
            for(i=0; i<0x80; i++)
                ram[1][i+0x80] = flashmem[(a&~0xff)|((a+i)&0xff)];
            /*
